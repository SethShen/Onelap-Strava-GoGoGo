...
class _SyncHistoryScreenState extends State<SyncHistoryScreen> {
  ...
  List<SyncRecord> get _filteredRecords {
    return _records.where((record) {
      ...
      if (!_showStrava && hasStrava) {
        return false;
      }
      if (!_showXingzhe && hasXingzhe) {
        return false;
      }
      ...
      final matchingResults = record.platformResults.where((r) {
        ...
        if (r.status == SyncStatus.success && !_showSuccess) {
          return false;
        }
        if (r.status == SyncStatus.failed && !_showFailed) {
          return false;
        }
        if (r.status == SyncStatus.deduped && !_showDeduped) {
          return false;
        }
        return true;
      }).toList();
